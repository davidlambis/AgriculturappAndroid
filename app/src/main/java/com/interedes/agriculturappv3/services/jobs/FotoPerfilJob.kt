...
class FotoPerfilJob : Job() {
    ...
    companion object {
        val TAG = "job_foto_perfil_job_sync"
        fun scheduleFotosJob() {
            val jobRequests = JobManager.instance().getAllJobRequestsForTag(FotoPerfilJob.TAG)
            if (!jobRequests.isEmpty()) {
                return
            }
            JobRequest.Builder(FotoPerfilJob.TAG)
    ...
    override fun onRunJob(params: Job.Params): Job.Result {
        repository?.syncFotoPerfilUserLogued(context)
